...
/*===========================================================================*/
  var client: EtcdClient =  new EtcdClient(URI.create("http://54.173.151.88:4001/"));
...
def setCounter(): String = {
	...
	println(client);
	var countValue: EtcdResult = null
	var res: EtcdResult = null
	var appkey: String = "/009404577/counter"
		...
		if(result == null){
				...
				var keyval: EtcdResult = this.client.set(appkey, "0");
		}
		else{
				var value = Integer.parseInt(result.node.value);
				var finalcount = value + 1;
				var keyval: EtcdResult = this.client.set(appkey, finalcount.toString());		
		}

			res =  this.client.get(appkey)
			println("res: " + res);
			var countervalue = Integer.parseInt(res.node.value)
